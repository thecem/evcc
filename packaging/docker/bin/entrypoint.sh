...
if [ -f "${HASSIO_OPTIONSFILE}" ]; then
  ...
  # and rename the old config file to .migrated
  if [ ! -f "${CONFIG}" ]; then
  ...
  # Only if the database file is located in '/config' instead of the default '/data',
  # migrate it as well
  ...
  # fallback for direct container start
  if [ "$1" = 'evcc' ]; then
    shift
    exec evcc "$@"
